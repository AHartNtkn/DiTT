...
| Standard library | `Language Core` | `tests/contracts_stdlib.rs`, `tests/contracts_stdlib_library.rs`, `tests/conformance/stdlib/*`, `tests/conformance/stdlib/api_lock.csv` | required stdlib modules are registry-backed, API-locked, parse/typecheck, and compose in acyclic dependency graph; library modules in `stdlib/` are registered, have bodies, define expected symbols, and typecheck |
| Normalizer substitution | `Type Theory` | `tests/contracts_normalizer_substitution.rs` | normalizer does not loop on self-named arguments, swapped argument names, or transitive substitution chains; stdlib compose_assoc and yoneda_ff_forward evaluate without overflow |
